...
$(document).ready(function(){
	...
	$("#kssj").val(getLastYear());
	$("#jssj").val(getNow());
	<c:if test="${command!=null}">
		$("#hphm").val("<c:out value='${command.hphm}'/>");
		$("#kssj").val("<c:out value='${command.kssj}'/>");
		$("#jssj").val("<c:out value='${command.jssj}'/>");
		$("#clzt").val("<c:out value='${command.clzt}'/>");
	</c:if>

});
function query(){
	if(!compareDate($("#kssj").val(),$("#jssj").val(),"开始时间","结束时间")) return false;
	$("#myform").attr("action","query.htm?method=listQueryFlow");
	closes();
	$("#myform").submit();
}
function detail(pk){
	openwin1024('flow.htm?method=detailQueryFlow&sqbh='+pk,'query');
}
</script>
</head>
<body>
				...
				<div class="row mb20">
					...
					<table border="0" cellspacing="1" cellpadding="0" class="query">
						...
						<tr>
							...
							<td class="body"><input type="text" class="form-control text-uppercase" name="hphm" id="hphm" maxlength="7"></td>
						...
						<tr>
							<td class="head">标识卡号</td>
							<td class="body"><input type="text" class="form-control text-uppercase" name="kh" id="kh" maxlength="12"></td>
							<td class="head">业务状态</td>
							<td class="body"><select id="clzt" name="clzt" class="select form-control">
								<option value="" selected="selected"></option>
								<option value="1">1:已录入</option>
								<option value="5">5:已通过</option>
								<option value="8">8:不通过</option>
								<option value="9">9:已作废</option>
							</select></td>
							<td class="command" colspan="2"><button type="button" class="btn btn-info" onClick="query()">查询</button><td>
						</tr>
					</table>
					</form>
				</div>
